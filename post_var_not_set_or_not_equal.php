...
print_header(
	'Benchmark',
	'post_var_not_set_or_not_equal',
	'Check If a Specific Variable in the <code>$_POST</code> Superglobal is Not Set or Not Equal to a Specific Value'
);
?>
	<h3>The Data Sets</h3>
...
unset( $_POST[ $key ] );
	...
	<pre>
/**
 * Check for nonexistence first, then for inequality.
 *
...
function algorithm_1( $key, $value ) {
	...
	<pre>
/**
 * Check if the filtered superglobal variable strictly compares to FALSE first, then maybe check for inequality.
 *
 * @param string $key   Desired array key.
 * @param string $value Comparison value.
 *
...
function algorithm_2( $key, $value ) {
	...
	return ( filter_input( INPUT_POST, $key ) === FALSE || filter_input( INPUT_POST, $key ) !== $value );
}
</pre>
	<h4>Algorithm 3</h4>
	<pre>
/**
 * Cache the filtered superglobal variable, and first check if it strictly compares to FALSE, then maybe check for inequality.
 *
 * @param string $key   Desired array key.
 * @param string $value Comparison value.
 *
...
function algorithm_3( $key, $value ) {
	...
	return ( ( $variable = filter_input( INPUT_POST, $key ) ) === FALSE || $variable !== $value );
}
</pre>
<?php
/**
